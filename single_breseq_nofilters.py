...
outdir = '/scicore/home/nimwegen/cappel0001/BZRS_PROJECT/mixed_samples_breseq/'+par_summary+'_nofilters/'+df.iloc[idx]['sample']+'/'+df.iloc[idx]['strain']+'/'+df.iloc[idx]['condition']
cmd = [
    "breseq", "-p", 
    "--polymorphism-score-cutoff", str(args.score_cutoff),
    "--polymorphism-frequency-cutoff", str(args.frequency_cutoff),
    ...
    "-r", str(df.iloc[idx]['ref_file']), 
    "-o", outdir,
    "-j", "1",
    str(df.iloc[idx]['fastq1']),
    str(df.iloc[idx]['fastq2'])
]
try:
